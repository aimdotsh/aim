...
懒猫 1.0.37 可以在“主机资源 → 添加受管主机”页面直接下载 Host Kit 2.4.14，也可以访问 `/downloads/aim-host-kit-2.4.14.tar.gz`。该下载仍受懒猫应用登录保护，压缩包内不包含用户私钥或服务器凭据。升级后应在每台目标机幂等重装一次 Host Kit；2.4.14 修复副本首次启动时 `super_read_only` 阻止 root 凭据和复制通道初始化的问题，不会删除已有 MySQL 数据。
