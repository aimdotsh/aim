...
while (( $# )); do
    case "$1" in
        ...
        --router-script) need_arg "$@"; ROUTER_SCRIPT="$2"; shift 2 ;;
        -h|--help) usage; exit 0 ;;
...
[[ -r "$ROUTER_SCRIPT" ]] || { printf 'router.sh is not readable: %s\n' "$ROUTER_SCRIPT" >&2; exit 1; }
...
AIM_UID="$(id -u "$AIM_USER")"
AIM_GID="$(id -g "$AIM_USER")"
...
install -o root -g root -m 0755 "$ROUTER_SCRIPT" /opt/aim/router.sh
...
cat >"$CONFIG_TMP" <<'EOF'
{
  ...
  "router_path": "/opt/aim/router.sh",
  "base_root": "/opt/mysql",
  "data_root": "/data/mysql",
  "log_root": "/var/log/mysql",
  "tmp_root": "/var/tmp/mysql",
  "staging_root": "/var/lib/aim-staging",
  "staging_uid": AIM_UID_PLACEHOLDER,
  "staging_gid": AIM_GID_PLACEHOLDER
}
EOF
# Substitute only the numeric identity values; the rest of the configuration
# remains a fixed, reviewable allowlist.
sed -i "s/AIM_UID_PLACEHOLDER/$AIM_UID/; s/AIM_GID_PLACEHOLDER/$AIM_GID/" "$CONFIG_TMP"
...
printf '  router.sh:      /opt/aim/router.sh\n'
