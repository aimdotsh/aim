...
Host Kit 2.4.14 是控制台 1.0.41 的配套版本。它修复副本首次启动时 `super_read_only` 过早生效、阻止 root 凭据和复制通道初始化的问题；安装和安全续跑都会在受控窗口内临时解除只读保护，完成后恢复 `read_only` 与 `super_read_only`，错误退出也会尽力恢复保护。重复安装只会更新受限工具，不会删除现有 MySQL 数据。
