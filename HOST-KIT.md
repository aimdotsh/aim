...
Host Kit 2.4.10 是控制台 1.0.28 的配套版本。它新增 OpenCloudOS 8/9 的 RHEL 兼容识别，同时会识别已配置但停止的 AIM 实例，修复早期失败部署留下的 `my.cnf` 属主和 MySQL 软件目录权限，兼容 Ubuntu 24.04 的 `libaio1t64` SONAME，并在 systemd 启动失败时立即回传诊断。它同时支持 MySQL Router；重复安装只会更新受限工具，不会删除现有 MySQL 数据。
