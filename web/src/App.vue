...
import { groupInstances, managedSourceInstances, topologyTypeLabel } from './instance-topology'
...
watch(() => deploy.mode, mode => {
  ...
  if (mode !== 'replica') sourceInstanceID.value = 0
})

watch(sourceInstanceID, id => {
  if (!id) return
      ...
      <div v-else-if="page === 'deploy'" class="page-stack">
        <section class="panel wizard"><div class="panel-head"><div><p class="eyebrow">DEPLOYMENT WIZARD</p><h3>创建 MySQL 部署任务</h3></div><span class="safety-badge"><ShieldCheck />先整体预检，再顺序执行</span></div>
          <form @submit.prevent="createDeployment">
            ...
            <fieldset v-if="['source','replica','replication'].includes(deploy.mode)"><legend><span>03</span>复制参数</legend><div class="form-grid"><label>复制账号<input v-model.trim="deploy.replication_user"></label><label v-if="deploy.mode === 'source'">允许的从库地址<input v-model.trim="deploy.replica_host"></label><label v-if="deploy.mode === 'replica'" class="span-2">从现有实例选择源库<select v-model.number="sourceInstanceID"><option :value="0">手工填写外部源库</option><option v-for="instance in sourceInstanceOptions" :key="instance.id" :value="instance.id">{{ instance.host_name }} · {{ instance.source_address || instance.address }}:{{ instance.port }} · {{ instance.role }}</option></select><small v-if="sourceInstanceOptions.length">选择后自动带入该受管实例的业务 IP 和 SQL 端口，仍可在下方手工修改。</small><small v-else>当前没有处于运行状态的受管 MySQL 实例，请手工填写可从目标从库访问的源库地址。</small></label><label v-if="deploy.mode === 'replica'">源库 IP<input v-model.trim="deploy.source_host" required><small>必须是目标从库能够直连的 MySQL 地址；公网 NAT 地址需确认回程和防火墙。</small></label><label v-if="deploy.mode === 'replica'">源库端口<input v-model.number="deploy.source_port" type="number" min="1" max="65535" required></label><label v-if="deploy.mode === 'replica'" class="span-2">源库复制密码<input v-model="deploy.source_password" type="password" required></label><label v-else class="span-2">复制密码（可自动生成）<input v-model="deploy.replication_password" type="password" placeholder="留空由控制台生成"></label></div></fieldset>
