...
import { clusterMemberLabel, clusterReplicas, clusterSource, groupInstances, managedSourceInstances, topologyTypeLabel } from './instance-topology'
...
const deploy = reactive({
  ...
  replication_user: 'aim_repl', source_instance_id: 0, source_host: '', source_port: 3306, source_password: '', replica_host: '%',
...
const instanceGroups = computed(() => groupInstances(instances.value))
const sourceInstanceOptions = computed(() => managedSourceInstances(
  instances.value,
  deploy.nodes[0]?.host_id,
  deploy.port
))
...
watch(() => deploy.mode, mode => {
  ...
  if (mode !== 'replica') deploy.source_instance_id = 0
})

watch(() => deploy.source_instance_id, id => {
  if (!id) return
  const instance = instances.value.find(item => Number(item.id) === Number(id))
  if (!instance) return
  deploy.source_host = instance.source_address || instance.address
  deploy.source_port = Number(instance.port)
})
...
function deploymentPayload() {
  ...
  payload.source_instance_id = Number(payload.source_instance_id) || 0
...
cd aim-host-kit-2.4.14</code></pre></div></article>
      ...
      <div v-else-if="page === 'deploy'" class="page-stack">
        <section class="panel wizard"><div class="panel-head"><div><p class="eyebrow">DEPLOYMENT WIZARD</p><h3>创建 MySQL 部署任务</h3></div><span class="safety-badge"><ShieldCheck />先整体预检，再顺序执行</span></div>
          <form @submit.prevent="createDeployment">
            ...
            <fieldset v-if="['source','replica','replication'].includes(deploy.mode)"><legend><span>03</span>复制参数</legend><div class="form-grid"><label>复制账号<input v-model.trim="deploy.replication_user"></label><label v-if="deploy.mode === 'source'">允许的从库地址<input v-model.trim="deploy.replica_host"></label><label v-if="deploy.mode === 'replica'" class="span-2">从现有实例选择源库<select v-model.number="deploy.source_instance_id"><option :value="0">手工填写外部源库</option><option v-for="instance in sourceInstanceOptions" :key="instance.id" :value="instance.id">{{ instance.host_name }} · {{ instance.source_address || instance.address }}:{{ instance.port }} · 受管主库</option></select><small v-if="sourceInstanceOptions.length">选择后会记录源实例关系，并自动归入同一套主从拓扑；业务 IP 和 SQL 端口由服务端再次核验。</small><small v-else>当前没有处于运行状态的受管主库，请手工填写可从目标从库访问的源库地址。</small></label><label v-if="deploy.mode === 'replica'">源库 IP<input v-model.trim="deploy.source_host" required><small>必须是目标从库能够直连的 MySQL 地址；选择受管主库时以服务端核验结果为准。</small></label><label v-if="deploy.mode === 'replica'">源库端口<input v-model.number="deploy.source_port" type="number" min="1" max="65535" required></label><label v-if="deploy.mode === 'replica'" class="span-2">源库复制密码<input v-model="deploy.source_password" type="password" required></label><label v-else class="span-2">复制密码（可自动生成）<input v-model="deploy.replication_password" type="password" placeholder="留空由控制台生成"></label></div></fieldset>
      ...
      <div v-else-if="page === 'clusters'" class="page-stack"><section class="panel"><div class="panel-head"><div><p class="eyebrow">TOPOLOGY</p><h3>真实复制、MGR 与 Router 拓扑</h3></div><span v-if="clusters.length" class="safety-badge"><Network />{{ clusters.length }} 套拓扑</span></div><div class="topology-list"><article v-for="cluster in clusters" :key="cluster.id" class="topology-card"><header><div><span class="topology-type">{{ topologyTypeLabel(cluster.type) }}</span><h3>{{ cluster.name }}</h3><small v-if="cluster.group_name" class="mono">组 UUID · {{ cluster.group_name }}</small></div><div class="topology-summary"><span class="status" :class="statusClass(cluster.state)">{{ cluster.state }}</span><strong>{{ cluster.online_members }}/{{ cluster.member_count }}</strong><small>在线节点</small></div></header><div v-if="cluster.type === 'replication'" class="replication-flow"><article v-if="clusterSource(cluster)" class="topology-node source-node"><span class="node-role-badge">主库</span><strong>{{ clusterSource(cluster).host_name }}</strong><code>{{ clusterSource(cluster).sql_endpoint }}</code><small>MySQL {{ clusterSource(cluster).version }}</small><span class="status" :class="statusClass(clusterSource(cluster).state)">{{ clusterSource(cluster).state }}</span></article><div class="replication-link"><span>GTID / Binlog</span><b>→</b></div><div class="replica-stack"><article v-for="member in clusterReplicas(cluster)" :key="member.id" class="topology-node replica-node"><span class="node-role-badge">从库</span><strong>{{ member.host_name }}</strong><code>{{ member.sql_endpoint }}</code><small>MySQL {{ member.version }}</small><span class="status" :class="statusClass(member.state)">{{ member.state }}</span></article><div v-if="!clusterReplicas(cluster).length" class="missing-node">尚无从库成员</div></div></div><div v-else class="mgr-topology"><div class="mgr-bus"><Network /><span>Group Replication</span><small>{{ cluster.group_name }}</small></div><div class="mgr-members"><article v-for="member in cluster.members" :key="member.id" class="topology-node mgr-node"><span class="node-role-badge">{{ clusterMemberLabel(member, cluster.type) }}</span><strong>{{ member.host_name }}</strong><code>SQL {{ member.sql_endpoint }}</code><code v-if="member.mgr_endpoint">MGR {{ member.mgr_endpoint }}</code><small>MySQL {{ member.version }}</small><span class="status" :class="statusClass(member.state)">{{ member.state }}</span></article></div></div><div v-if="cluster.router_enabled" class="router-strip"><div><Network /><span><strong>MySQL Router · {{ cluster.router_cluster_name }}</strong><small>Classic RW :{{ cluster.router_rw_port }}</small></span></div><code>{{ cluster.router_endpoints.join(' · ') }}</code></div><footer><span>拓扑创建于 {{ formatTime(cluster.created_at) }}</span><span v-if="cluster.state === 'degraded'">成员数量或运行状态不完整，请检查节点</span></footer></article><div v-if="!clusters.length" class="empty-card">暂无集群拓扑；仅主库与后来选择它创建的从库会自动合并显示。</div></div></section></div>
