...
func TestProbeDetectsStoppedAIMInstance(t *testing.T) {
	dataRoot := t.TempDir()
	port := 3319
	instanceDir := filepath.Join(dataRoot, "3319")
	if err := os.MkdirAll(instanceDir, 0o750); err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile(filepath.Join(instanceDir, "my.cnf"), []byte("# Generated by AIM 2.3.1; instance 3319, MySQL 8.0.46.\n[mysqld]\n"), 0o640); err != nil {
		t.Fatal(err)
	}
	facts, err := ProbeHost([]int{port}, dataRoot)
	if err != nil {
		t.Fatal(err)
	}
	if facts.Ports[port] != "available" || facts.AIMInstances[port] != "configured" {
		t.Fatalf("stopped AIM instance was not detected: ports=%v instances=%v", facts.Ports, facts.AIMInstances)
	}
	if err := os.WriteFile(filepath.Join(instanceDir, "my.cnf"), []byte("[mysqld]\nport=3319\n"), 0o640); err != nil {
		t.Fatal(err)
	}
	facts, err = ProbeHost([]int{port}, dataRoot)
	if err != nil {
	...
	if facts.AIMInstances[port] != "" {
		t.Fatal("an unmanaged configuration was accepted as an AIM resume candidate")
	}
}
