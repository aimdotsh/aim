...
- 将任务暂存目录设置为 `aimops` 可穿越的 `0750`，并把生成的备份文件设置为 `aimops:aimops`、`0600`，仅允许控制台通过 SFTP 下载。

控制台升级后必须使用同一版本的新版 host kit 重新执行一次安装。重新执行是幂等的，不会删除 MySQL 数据；它会替换 root-owned 执行器并更新 `/etc/aim/executor.json`。如果仍使用旧执行器，部署可以继续工作，但“备份中心”和“健康监控”会因为目标机不认识 `backup`/`metrics` action 而失败。
...
如果决定放弃失败部署，管理员可点击“清理失败安装”。控制台先在所有部署节点运行只读预览，预览成功后要求输入任务专属确认文本，再逐台停止并删除该端口的 AIM 数据、日志和 systemd 服务。`/opt/mysql/<version>` 仅在没有其他 AIM 配置引用时删除；系统共享依赖包和安装介质不会被删除。任何主机清理失败都会保留失败日志，其他主机仍继续清理。

“主机资源”允许删除没有受管 MySQL 实例、且没有运行中或待核实任务的主机，不要求目标机当前可连接。该操作只删除控制台主机记录和加密 SSH 私钥，不连接远端；历史任务的日志和规格继续保留。存在受管实例时必须先在“MySQL 实例”页执行卸载。
...
备份是面向 InnoDB 的一致性逻辑备份；MyISAM 等非事务表不能保证同一时间点。生产环境建议使用专用、最小权限的备份账号；当前自动部署流程使用已加密保存的 root 凭据，密码只在执行器的短暂任务期间解密。
