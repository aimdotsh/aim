...
重新执行新版 Host Kit 是幂等的，不会删除 MySQL 数据。升级到控制台 1.0.41 后，应在每台目标机重新执行一次 2.4.14。该版本会在副本 root 凭据和复制通道初始化期间临时解除只读保护，并在成功或错误退出时恢复 `read_only` 与 `super_read_only`，修复首次搭建主从时报错 `ERROR 1290` 的问题。
