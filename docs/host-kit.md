# AIM Host Kit 2.4.5（部署、断点恢复、备份、监控与 Router）

控制台会通过 SSH 连接目标机的 `aimops` 用户。Host Kit 2.4.5 安装 MySQL 生命周期脚本、MySQL Router 脚本、受限执行器，以及安全续跑、在线备份和健康监控所需的协议动作。
...
重新执行新版 Host Kit 是幂等的，不会删除 MySQL 数据。升级到控制台 1.0.22 后，必须在每台目标机重新执行一次 2.4.5；否则“从失败节点重试”无法识别新的安全续跑动作。
