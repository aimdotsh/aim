...
重新执行新版 Host Kit 是幂等的，不会删除 MySQL 数据。升级到控制台 1.0.34 后，应在每台目标机重新执行一次 2.4.13。该版本会修正已解压的 Router 目录权限，避免 `mysqlrouter` 服务用户触发 `203/EXEC Permission denied`，并在 Router 端口未就绪时输出 systemd 与 journal 诊断。
